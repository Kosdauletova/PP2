...
# Функция для проверки, имеет ли фильм рейтинг IMDB выше 5,5
def is_highly_rated(movie):
    return movie["imdb"] > 5.5

# Функция для просмотра фильмов с рейтингом IMDB выше 5,5
def highly_rated_movies(movies):
    return [movie for movie in movies if movie["imdb"] > 5.5]

# Функция поиска фильмов по категориям
def movies_by_category(movies, category):
    return [movie for movie in movies if movie["category"] == category]

# Функция для вычисления средней оценки списка фильмов на IMDB
def average_imdb(movies):
    if not movies:
        return 0
    return sum(movie["imdb"] for movie in movies) / len(movies)

# Функция для вычисления среднего балла IMDB за фильмы в данной категории
